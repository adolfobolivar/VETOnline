...
- **Imperative Navigation vs. `RequireAuth`'s Reactive Redirect:** Any explicit action that both changes auth state and navigates (logout is the current example) must navigate *before* clearing the session, not after. `RequireAuth` renders `<Navigate to="/login">` reactively the moment `isAuthenticated` flips to `false` — if that state change happens while still mounted under a protected route, `RequireAuth`'s redirect can race an explicit `navigate("/")` call and win, stranding the user on the login form instead of wherever the action was actually supposed to land them (discovered via UC-011 A3: logging out from a protected screen intermittently landed on `/login` instead of the welcome page). The fix is ordering, not a synchronization primitive: trigger the intended navigation first, so the component tree is no longer under `RequireAuth` by the time the auth-clearing state update propagates.
...
- **Migration Package Naming:** The migration Lambda's deployment package cannot contain a directory named `alembic/` for the migration scripts — `uv pip install --target` installs the **pip package** `alembic` (the library itself) at exactly that path, so copying the project's own migrations there merges into the installed library instead of replacing it, nesting `env.py` one level too deep and breaking `alembic upgrade head` at runtime with `Can't find Python file .../alembic/env.py`. The migration package copies migrations to a differently-named directory (`migrations/`) and points `script_location` at it explicitly; this is purely a packaging-step concern; `backend/alembic/` remains the correct, uncollided name for local development (`uv run alembic upgrade head`), since the pip package lives in `.venv/` there, not alongside the project's own migrations.
...
- **Ad Hoc Access (RDS Data API):** The database can also be queried ad hoc for development purposes through RDS Data API — a separate, IAM-authenticated AWS interface, not a direct network connection. This doesn't change the private, no-public-access posture described above; it's simply a second, identity-gated way in, alongside the application's own Lambda-based connections.
